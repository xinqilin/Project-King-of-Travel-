...
<%
	...
	for(int i =0;i<wishList.size();i++){
		if((wishList.get(i)).getBuyOrSell()==0&&(wishList.get(i)).getMemNo().equals(memNo)){
			wishBuyList.add(wishList.get(i));
		};
		if((wishList.get(i)).getBuyOrSell()==1&&(wishList.get(i)).getMemNo().equals(memNo)){
			wishSellList.add(wishList.get(i));
		};		
	};
		...
		<div class="row">
			<div class="col-3">
				<div class="nav flex-column nav-pills" id="v-pills-tab"
					...
					<a class="nav-link active" href="<%=request.getContextPath()%>/front-end/WishItem/listMyWishItem.jsp">我的託買及代購商品</a>
			...
			<%@ include file="wishBuyListPage1.file" %>			
				...
				<div class="card" style="width: 16rem;">
					...
					<div class="card-body">
    					...
    					<p class="card-text">代購還是託買:&nbsp&nbsp
    					<c:if test="${wishItemVO.buyOrSell==0}">代購</c:if>
    					<c:if test="${wishItemVO.buyOrSell!=0}">託買</c:if>
    					</p>
				...
				<form  METHOD="post" ACTION="wishItem.do">
				<div class="card" style="width: 16rem;">
 					<img src="<%=request.getContextPath()%>/getDatabaseServlet.do?wishItemNo=${wishItemVO.wishItemNo}" class="card-img-top" alt="...">
					<div class="card-body">
    					<h5 class="card-title">商品名稱:${wishItemVO.itemName}</h5>
    					<p class="card-text">商品簡述:${wishItemVO.wishItemDetail}</p>
    					<input type="hidden" name="wishItemNo" value="${wishItemVO.wishItemNo}">
