...
<br>
	...
	<div class="row" style="margin-left:3%;">
		...
		<br>
			...
			<input type="button" class="onme" value="跳到第<%=p %>天" style="background-color: #AAAAAA" disabled="disabled"></input>
			<br><br>
			<%}j=1; %>
...
<input type="number" name="time22" min="0" max="59" 
...
style=" border: none;background-color:white;text-align: center;color: #AAAAAA;" class="timee" required id="qq<%=++qq%>">:
<input type="number" name="time44" min="0" max="59" 
...
<center><button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#addspot" aria-expanded="false" aria-controls="collapseExample">
   新增景點 點我!
	...
	<div class="collapse" id="addspot" data-parent="#accordionExample">
  <div class="card card-body">	
							...
							<b>選擇加入第幾天:</b> 
									...
									&nbsp&nbsp
							<input type="radio" name="tripdayorder" value="<%=q%>" <%if(q==1){ %>checked<%} %>><b><%=q%></b>
...
&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<b>寫些小記  &nbsp:&nbsp&nbsp</b><input   name="triptips" value="" type="text" class="text2" style="background-color: white;">
					...
					<br><img src="<%=request.getContextPath()%>/front-end/tripDetails/pic/plus.png" style="width:25px;height:25px;">
					<input type="submit" value="新增這個景點到行程">
							<%} %>
						</form>
	</div></div>
				          <hr color="#99FF99" width="80%"/>
...
<%
	java.sql.Time timeofarrive = null;
	try {
		timeofarrive = tripDetailsVO.getTimeofarrive();
	} catch (Exception e) {
		timeofarrive = new java.sql.Time(System.currentTimeMillis());
	}
%>
<%
	java.sql.Time timeofleave = null;
	try {
		timeofleave = tripDetailsVO.getTimeofleave();
	} catch (Exception e) {
		timeofleave = new java.sql.Time(System.currentTimeMillis());
	}
%>
